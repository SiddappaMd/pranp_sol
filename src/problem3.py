...
df['maximum_temp']=list(map(float,df['maximum_temp'].values))
df['minimu_temp']=list(map(float,df['minimu_temp'].values))
...
for year in result.index:
	max_count = result['maximum_temp'].loc[year]
	min_count = result['minimu_temp'].loc[year]
...
f.close()
print("Successfully generated the output for problem 2 in answers folder: In YearHistogram.out")
